...
export default function SignupScreen() {
  ...
  const promptForPermission = useNotificationStore((s) => s.promptForPermission);
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      // Prompt for notification permission after successful signup
      // This shows the native system permission dialog
      Alert.alert(
        'Enable Notifications?',
        'Stay updated with new messages, application status updates, and important alerts from MortgageConnect.',
        [
          {
            text: 'Not Now',
            style: 'cancel',
            onPress: () => {
              console.log('[Signup] User declined notification prompt');
            },
          },
          {
            text: 'Enable',
            onPress: async () => {
              console.log('[Signup] User agreed to enable notifications');
    ...
    <SafeAreaView className={`flex-1 ${isDark ? 'bg-black' : 'bg-gray-50'}`}>
      ...
      <KeyboardAwareScrollView
        ...
        <Animated.View entering={FadeInDown.delay(200).duration(600)}>
          ...
          <View className="mb-4">
            ...
            <View
              ...
              <TextInput
                ...
                className={`ml-3 flex-1 py-3.5 text-base ${isDark ? 'text-white' : 'text-black'}`}
              />
            </View>
            {errors.displayName && (
          ...
          {/* Terms text */}
          <Text
            className={`mt-4 text-center text-xs leading-5 ${isDark ? 'text-gray-600' : 'text-gray-400'}`}>
          ...
          <View className="my-6 flex-row items-center">
            <View className={`h-px flex-1 ${isDark ? 'bg-[#333]' : 'bg-gray-200'}`} />
            <Text className={`mx-4 text-sm ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>or</Text>
            <View className={`h-px flex-1 ${isDark ? 'bg-[#333]' : 'bg-gray-200'}`} />
          </View>
